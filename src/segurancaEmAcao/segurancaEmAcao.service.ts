...
@Injectable()
export class SegurancaEmAcaoService {
  ...
  async create(body: any, imagem?: Express.Multer.File) {
    ...
    const dadosImagem = imagem
      ? {
          ...
          caminho: `/downloads/seguranca-em-acao/${imagem.filename}`,
    ...
    try {
      return await this.prisma.segurancaEmAcao.create({
        data: {
  ...
  async findByFilter(body: any) {
    ...
    const [segurancaEmAcaos, total] = await this.prisma.$transaction([
      this.prisma.segurancaEmAcao.findMany({
        where,
        skip,
        take: limitNumber,
        orderBy: {
      ...
      this.prisma.segurancaEmAcao.count({
        where,
      }),
    ]);

    return {
      data: segurancaEmAcaos,
      total,
      page: pageNumber,
      limit: limitNumber,
  ...
  async findAllAtivos() {
    return this.prisma.segurancaEmAcao.findMany({
      where: {
        ativo: true,
      },
      orderBy: {
  ...
  async findById(id: string) {
    ...
    const segurancaEmAcao = await this.prisma.segurancaEmAcao.findUnique({
      where: {
        id,
      },
    });
  ...
  async update(body: any, novaImagem?: Express.Multer.File) {
    ...
    const segurancaEmAcaoAtual = await this.findById(id);
    ...
    const dadosNovaImagem = novaImagem
      ? {
          ...
          nomeSalvo: novaImagem.filename,
          caminho: `/downloads/seguranca-em-acao/${novaImagem.filename}`,
    ...
    try {
      const segurancaEmAcaoAtualizado =
        await this.prisma.segurancaEmAcao.update({
          where: {
            id,
          },
          data: {
       ...
       */
      if (segurancaEmAcaoAtual.imagem && (novaImagem || deveRemoverImagem)) {
        try {
          this.removerArquivoImagem(segurancaEmAcaoAtual.imagem);
        } catch (error) {
          console.error('Não foi possível remover a imagem anterior:', error);
        }
      }

      return segurancaEmAcaoAtualizado;
    } catch (error: any) {
      /*
  ...
  async alterarStatus(body: any) {
    ...
    return this.prisma.segurancaEmAcao.update({
      where: {
        id,
      },
      data: {
  ...
  async delete(id: string) {
    ...
    await this.prisma.segurancaEmAcao.delete({
      where: {
        id,
      },
    });
