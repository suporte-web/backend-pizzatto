...
export class RecrutamentoInternoCandidatoController {
  ...
  @ApiOperation({summary: 'Busca as candidaturas feitas pelo ID do Recrutamento'})
  async findByRecrutamento(@Param('idRecrutamento') idRecrutamento: string) {
    return await this.recrutamentoInternoCandidatoService.findByRecrutamento(idRecrutamento)
  }
  
  @Get('find-by-candidato/:idCandidato')
  @ApiOperation({summary: 'Busca as candidaturas feitas pelo ID do Candidato'})
  async findByCandidato(@Param('idCandidato') idCandidato: string) {
    return await this.recrutamentoInternoCandidatoService.findByCandidato(idCandidato)
  }
